...
const addCommonFieldsToInitialDataMiddleware = () => ({ getState, dispatch }) => next => action => {
  ...
  const cmDataStore = getState().get('content-manager-mock-mock_editViewCrudReducer');
  const cmLayoutStore = getState().get('content-manager-mock-mock_editViewLayoutManager');
  ...
  const getData = async () => {
    ...
    try {
      const requestURL = `/${pluginId}/content-manager-mock/actions/get-non-localized-fields`;
